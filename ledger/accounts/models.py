...
class Profile(RevisionedMixin):
    ...
    institution = models.CharField('Institution', max_length=200, blank=True, default='',
        ...
        if instance.email:
            if EmailIdentity.objects.filter(email=instance.email).exclude(user=instance.user).exists():
                ...
                raise ValidationError(
                    "This email address is already associated with an existing account or profile; if this email address belongs to you, please contact the system administrator to request for the email address to be added to your account.")
