...
// on form submit
function bindForm(my_form_id,input_id,upload_type) {
		...
		if (xhr.upload) {
			xhr.upload.addEventListener('progress', function(event) {
				...
				if (percent > 99) { 
					...
					$(progress_bar_id + " .status-text").text("success");
				} else {
...
var ajax_loader_django = { 
	...
	var htmlvalue = "";
	    ...
	    htmlvalue += '  class="custom-file-input"';
            ...
            htmlvalue += '</label>';
            htmlvalue += '<input name="__submit__" type="submit" class="btn btn-primary" value="Upload"/>';
            htmlvalue += '</form>';
	    htmlvalue += '<BR><BR>';
           
	    htmlvalue += '<div id="progress-wrp" class="progress">';
            htmlvalue += '  <div id="progress-bar-indicator" class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:0%">';
            htmlvalue += '    <span class="status">0%</span> Complete (<span class="status-text">success</span>) ';
            htmlvalue += '  </div> ';
            htmlvalue += '</div>';
            htmlvalue += '<BR><BR>';
            htmlvalue += '<div istyle="float:left; " id="showfiles"></div>';
